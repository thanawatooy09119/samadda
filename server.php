...
if($message != "")
{
 $sql = "INSERT INTO `chat` VALUES(NULL,'$message','customer','$_myid')";
 mysql_query($sql);

 
 $sql = "UPDATE `customer` SET `read` = '1' WHERE `id_customer` = '$_myid' LIMIT 1;";
 mysql_query($sql);
}
...
while($row = mysql_fetch_array($result)){
    if($row['type']=="admin"){
        ...
        echo $row['Text'];
        echo '<span class="inchat"><i class="fa fa-user-circle" aria-hidden="true"></i> seller:</span>';
        echo "</div>";
