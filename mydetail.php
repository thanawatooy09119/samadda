...
<tr>
    <td colspan="2">ใบเสร็จร้าน สมัดดาผ้าทอมือ <br>
    ที่ตั้งร้าน 109 หมู่ 6 บ้านหนองสูงใหม่ ตำบล หนองสูงเหนือ <br>
    อำเภอหนองสูง จังหวัดมุกดาหาร 49160
    ...
    <td colspan="2">
             <a href="javascript:print();" class="dontshowprn ibtn-orange">พิมพ์</a>
    </td>        
</tr>
<tr>
<td colspan="4"><hr /></td>
...
$_qty  = i_result($on,$x,qty_detail);
$_price = i_result($on,$x,price_product);
$_sum = 0;
$_sum = $_price*$_qty;
$sum_qty += $_qty;
$sum_all = $sum_all+$_sum;
...
echo '</li></ul></div></div>';   
echo'</td>';
echo '<td>'.number_format($_qty).'</td>';
echo '<td>'.number_format($_sum).'</td>';
echo '</tr>';
...
echo '<tr>';
echo '<td> </td>';
echo '<th> รวมทั้งสิ้น </th>';
echo '<td>'.number_format($sum_qty).'</td>';
echo '<th >'.number_format($sum_all).'</th>';
...
<th colspan="4">
<hr>หากลูกค้าได้รับสินค้ากรุณาถ่าย วิดีโอเก็บไว้เป็นหลักฐานเมื่อเกิดสินค้าเสียหายจะได้นำมาเปลี่ยนได้
